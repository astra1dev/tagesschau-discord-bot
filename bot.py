...
async def get_news() -> discord.Embed or None:
    """
    Fetches the latest news from the tagesschau API
...
async def news(interaction: discord.Interaction) -> None:
    """
    Responds to the interaction with the latest news in an embed
    :param interaction: The discord interaction object
    :return: None
    """
    news_res = await get_news()
    if news_res is None:
        embed = discord.Embed(title=":x: Error", colour=discord.Colour(0xff0000), description="No new news available")
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    else:
        await interaction.response.send_message(embed=news_res)


@tasks.loop(minutes=TIME_TO_CHECK)
async def newsfeed() -> None:
    """
    Sends the latest news in an embed into a pre-defined channel
    :return: None
    """
    ...
    if news_res is None:
        ...
        await feedchannel.send(embed=news_res)
